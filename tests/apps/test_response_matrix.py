...
# ---------------------------------------------------------------------------
# set_rf_response behavior
# ---------------------------------------------------------------------------

class TestSetRfResponse:
    """Tests for set_rf_response() — auto/explicit weight and cache invalidation."""

    def _make_rm_with_rf(self, n_outputs=10, n_inputs=6, rf_response=None, rf_weight=None):
        """Helper to build a ResponseMatrix with rf_response for set_rf_response tests."""
        ...
        kwargs = dict(
            matrix=matrix,
            rf_response=rf_response,
            input_names=[f'cor_{i}' for i in range(n_inputs)],
            output_names=[f'bpm_{i}' for i in range(n_outputs)],
        )
    ...
    def test_explicit_weight_preserved(self):
        explicit_weight = 5.0
        rm = self._make_rm_with_rf(rf_weight=explicit_weight)
    ...
    def test_cache_invalidated_after_set_rf_response(self):
        rm = self._make_rm_with_rf()
